...
resource "aws_route53_record" "cert_validation" {
  ...
  name            = tolist(aws_acm_certificate.cf_alias[each.key].domain_validation_options)[0].resource_record_name
  records         = [tolist(aws_acm_certificate.cf_alias[each.key].domain_validation_options)[0].resource_record_value]
  type            = tolist(aws_acm_certificate.cf_alias[each.key].domain_validation_options)[0].resource_record_type
  zone_id         = each.value
  ttl             = 60
...
resource "aws_acm_certificate_validation" "cert" {
  ...
  certificate_arn         = aws_acm_certificate.cf_alias[each.key].arn
